...
public class TenPayApiV3Controller : BaseController
{
        ...
        /// TODO：可以放入缓存，设置有效时间
        /// </summary>
        public static readonly ConcurrentDictionary<string, string> TradeNumberToTransactionId = new();
        ...
        public IActionResult Index(int productId = 0, int hc = 0)
        {
                ...
                string? url = OAuthApi.GetAuthorizeUrl(TenPayV3Info.AppId, returnUrl, state, OAuthScope.snsapi_userinfo);//   -- DPBMARK MP DPBMARK_END
        ...
        public async Task<IActionResult> JsApi(int productId, int hc)
        {
                try
                {
                        ...
                        HttpContext.Session.SetString("BillNo", sp_billno ?? string.Empty);
        ...
        public async Task<IActionResult> Refund()
        {
                try
                {
                        ...
                        var outRefundNo = "OutRefunNo-" + SystemTime.Now.Ticks;
                        _ = int.TryParse(HttpContext.Session.GetString("BillFee"), out int totalFee);
