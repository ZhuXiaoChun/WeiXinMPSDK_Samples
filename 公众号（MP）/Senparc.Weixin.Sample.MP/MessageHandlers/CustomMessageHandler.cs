...
public partial class CustomMessageHandler
        /* 如果不需要自定义，可以直接使用：MessageHandler<DefaultMpMessageContext> */
        : MessageHandler<DefaultMpMessageContext>
{
	 ...
	 */
        ...
        private readonly string _appId = Config.SenparcWeixinSetting.MpSetting.WeixinAppId;
        //private readonly string _appSecret = Config.SenparcWeixinSetting.MpSetting.WeixinAppSecret;

        /// <summary>
        /// 为中间件提供生成当前类的委托
        /// </summary>
#pragma warning disable IDE0052 // 删除未读的私有成员
        public static readonly Func<Stream, PostModel, int, IServiceProvider, CustomMessageHandler> GenerateMessageHandler
                ...
                RequestMessageText requestMessage)
        {
                ...
                var requestHandler = await requestMessage.StartHandler()
                    ...
                    .Keyword("OPENID", () =>
                    {
                            ...
                            var userInfo = Weixin.MP.AdvancedAPIs.UserApi.Info(_appId, openId, Language.zh_CN);

#pragma warning disable CS0618 // 类型或成员已过时
                            defaultResponseMessage.Content = string.Format("您的OpenID为：{0}\r\n昵称：{1}\r\n性别：{2}\r\n地区（国家/省/市）：{3}/{4}/{5}\r\n关注时间：{6}\r\n关注状态：{7}\r\n\r\n说明：从2021年12月27日起，公众号无法直接获取用户昵称、性别、地区等信息，如需获取相关信息，需要使用<a href=\"https://sdk.weixin.senparc.com/oauth2?returnUrl=%2FOAuth2%2FTestReturnUrl\">OAuth 2.0 接口</a>。",
                                    requestMessage.FromUserName,
                                    userInfo.nickname,
                                    (WeixinSex)userInfo.sex,
                                    userInfo.country,
                                    userInfo.province,
                                    userInfo.city,
                                    DateTimeHelper.GetDateTimeFromXml(userInfo.subscribe_time),
                                    userInfo.subscribe);
#pragma warning restore CS0618 // 类型或成员已过时
        ...
        /// <returns></returns>
        public override async Task<IResponseMessageBase> OnLocationRequestAsync(
        ...
        public override async Task<IResponseMessageBase> OnShortVideoRequestAsync(
        ...
        /// <param name="requestMessage"></param>
        /// <returns></returns>
        public override async Task<IResponseMessageBase> OnImageRequestAsync(
                RequestMessageImage requestMessage)
        {
                IResponseMessageBase responseMessage;
                //一隔一返回News或Image格式
                if (GlobalMessageContext
                        .GetMessageContext(requestMessage)
                        .RequestMessages.Count % 2 == 0)
                {
                ...
                {
                        var responseMessageImage = CreateResponseMessage<ResponseMessageImage>();
        ...
        /// <param name="requestMessage"></param>
        /// <returns></returns>
        public override async Task<IResponseMessageBase> OnVoiceRequestAsync(
                RequestMessageVoice requestMessage)
        {
                var responseMessage = CreateResponseMessage<ResponseMessageMusic>();
                //上传缩略图
                //var accessToken = Containers.AccessTokenContainer.TryGetAccessToken(_appId, _appSecret);
                var uploadResult = Weixin.MP.AdvancedAPIs
                        .MediaApi
        ...
        /// <param name="requestMessage"></param>
        /// <returns></returns>
        public override async Task<IResponseMessageBase> OnVideoRequestAsync(
                ...
                _ = Task.Factory.StartNew(async () =>
                   {
                           ...
                           await CustomApi.SendVideoAsync(_appId, base.OpenId, uploadResult.media_id, "这是您刚才发送的视频", "这是一条视频消息");
                   }).ContinueWith(async task =>
                   {
                           if (task.Exception != null)
                           {
                                  ...
                                  task.Exception.InnerException?.Message);
                                   await CustomApi.SendTextAsync(_appId, base.OpenId, msg);
                           }
                   });
        ...
        /// <param name="requestMessage"></param>
        /// <returns></returns>
        public override async Task<IResponseMessageBase> OnLinkRequestAsync(
                RequestMessageLink requestMessage)
        {
                var responseMessage = ResponseMessageBase
                        .CreateFromRequestMessage<ResponseMessageText>(requestMessage);
        ...
        public override async Task<IResponseMessageBase> OnFileRequestAsync(
                RequestMessageFile requestMessage)
        {
                ...
                responseMessage.Content = string.Format(
                        ...
                        requestMessage.FileTotalLen,
                        requestMessage.FileMd5);
                //
                await Task.CompletedTask;
                return responseMessage;
                //
        ...
        /// <param name="requestMessage"></param>
        /// <returns></returns>
        public override async Task<IResponseMessageBase> OnEventRequestAsync(
        ...
        public override async Task<IResponseMessageBase> DefaultResponseMessageAsync(
        ...
        public override IResponseMessageBase DefaultResponseMessage(
                IRequestMessageBase requestMessage)
        {
		...
		*/
		...
		//为了在分布式缓存下提高读写效率，使用此方法，如果需要获取实时数据，应该使用 base.GetCurrentMessageContext()
		var currentMessageContext = await base.GetUnsafeMessageContext();
