...
public partial class WeixinController : BaseController
{
        ...
        readonly Func<string> _toGetRandomFileName
                = () =>
                {
                        return SystemTime.Now.ToString("yyyyMMdd-HHmmss")
        ...
        public new static readonly string AppId = MpSetting.WeixinAppId!;//与微信公众账号后台的AppId设置保持一致，区分大小写。
        ...
        /// <summary>
        /// 微信后台验证地址（使用Get），微信后台的“接口配置信息”的Url填写如：#{当前服务_根路径}#/weixin
        /// </summary>
        [HttpGet]
        [ActionName("Index")]
        public ActionResult IndexByGet(PostModel postModel, string echostr)
        {
        ...
        [HttpPost]
        [ActionName("Index")]
        public async Task<ActionResult> IndexByPost(PostModel postModel)
        {
                if (!CheckSignature.Check(postModel.Signature, postModel.Timestamp, postModel.Nonce, Token))
                {
                        return Content("参数错误！");
		 ...
		 * 收到重复消息通常是因为微信服务器没有及时收到响应，会持续发送2-5条不等的相同内容的 RequestMessage */
                ...
                catch (Exception ex)
                {
                        ...
                        using (TextWriter tw = new StreamWriter(ServerUtility.ContentRootMapPath("~/App_Data/Error_" + _toGetRandomFileName() + ".txt")))
                        {
